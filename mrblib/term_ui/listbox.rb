module TermUI
  ...
  class Listbox < Pack
    ...
    on_event(:key_down) do |event|
      self.selected_index -= 1 if event.key == Termbox::KEY_ARROW_UP && selected_index != 0
      self.selected_index += 1 if event.key == Termbox::KEY_ARROW_DOWN && selected_index != children.count-1
      
      if event.key == Termbox::KEY_ENTER
