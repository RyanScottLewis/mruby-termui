module TermUI
  ...
  class Textbox < Label
    ...
    on_event(:key_down) do |event|
      if event.target == self
        self.text = text[0..-2] if [Termbox::KEY_BACKSPACE, Termbox::KEY_BACKSPACE2].include?( event.key )
        self.text << " " if event.key == Termbox::KEY_SPACE
        # self.text << "\n" if event.key == Termbox::KEY_ENTER # TODO: if multiline?
    ...
    def set_cursor_position
      ...
      cursor.y = absolute_y+computed_height-1
    end
